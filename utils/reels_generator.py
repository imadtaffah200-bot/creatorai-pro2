...
class منشئ_الريلز:
    ...
    def إنشاء_فيديو_نصي(self, العنوان, النص, ملف_صوت=None):
        ...
        for شريحة in الشرائح:
            المقاطع.append(ImageClip(str(شريحة)).set_duration(3).resize(height=1920))
        الفيديو = concatenate_videoclips(المقاطع, method="compose")
        if ملف_صوت:
            الصوت = AudioFileClip(ملف_صوت)
            الفيديو = الفيديو.set_audio(الصوت).set_duration(min(الفيديو.duration, الصوت.duration))
        المسار = self.مجلد_الإخراج / f"reel-{int(time.time())}.mp4"
        الفيديو.write_videofile(str(المسار), fps=24, codec="libx264", audio_codec="aac", verbose=False, logger=None)
        return المسار

    def _إنشاء_شرائح(self, العنوان, النص):
        الجمل = [جملة.strip() for جملة in النص.replace("؟", ".").replace("!", ".").split(".") if جملة.strip()]
        الشرائح = []
    ...
    def _رسم_شريحة(self, النص, المسار, رقم):
        ...
        الرسم.rounded_rectangle((70, 90, العرض - 70, الارتفاع - 90), radius=60, outline=ألوان[رقم % len(ألوان)], width=10)
        الخط = self._خط(68)
        y = 620
        for سطر in self._تقسيم(النص, 18)[:7]:
            ...
            y += 94
        الرسم.text((90, الارتفاع - 190), "مساعد صانع المحتوى العربي", font=self._خط(38), fill="#d1d5db")
